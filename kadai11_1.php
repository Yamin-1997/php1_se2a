...
try {
  ...
  //to do ステートメント変数　　とsqlセット


  $db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  $db->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);

  $sql = "SELECT * FROM " . TBL_PRODCUT . " WHERE product_no = :product_no";
  $stmt = $db->prepare($sql); // sql form
  //プレイスタイルダーニパラメダをバインド
  $stmt->bindValue(":product_no", $product_no, PDO::PARAM_INT);

  // SQLを実行する
  $stmt->execute(); // active to stmt 
        ...
        <div class="p-5 row">
          <div class="col-md-5">
            ...
            <div class="col">
              ...
              <input type="text" name="product_no" id="product_no" class="form-control form-control-lg border-info bg-light" value="<?= $result['product_no'] ?>" readonly>
            </div>
            ...
            <div class="row">
              <div class="col">
                ...
                <select class="form-select form-select-lg mb-3" name="category">
                  <option value="ピザ"
                    <?= $result['category'] == "ピザ" ? 'selected' : '' ?>>
                    ピザ
                  </option>
                  <option value="ドリンク"
                    <?= $result['category'] == "ドリンク" ? 'selected' : '' ?>>
                    ドリンク
                  </option>
                </select>
              </div><!-- .col -->
