...
// $postData name, kana が空白でなければ、$viewDataに格納
if ($postData["name"]) {
...
// 「学科」と「コース」はそれぞれkadai03_resource.phpの$departmentsと$coursesから該当する数値を判別
$departmentName = '';
foreach ($departments as $dep) {
    if ($dep['d_id'] === $postData["department"]) {
        $departmentName = $dep['d_name'] . '（' . $dep['d_years'] . '年制）';
    }
}

$courseName = '';
foreach ($courses as $cou) {
    if ($cou['c_id'] === $postData["course"]) {
        $courseName = $cou['c_name'];
    }
}
